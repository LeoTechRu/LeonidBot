...
- Добавлена автоматическая подстановка SQLite при отсутствии переменных БД, благодаря чему приложение стартует без PostgreSQL.
- `.env.example` дополнен примерами переменных для настройки базы данных.
...
- При необходимости переключать базу данных на PostgreSQL через `DATABASE_URL` и подготовить миграции.
