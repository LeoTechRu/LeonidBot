...
- Добавлено автоматическое переключение на SQLite при недоступности PostgreSQL,
  что позволяет запускать веб-приложение без внешней БД.
...
- Настроить подключение к реальной PostgreSQL в продуктивной среде вместо временной SQLite.
